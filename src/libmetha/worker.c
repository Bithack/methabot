...
lm_worker_sort(worker_t *w)
{
    ...
    int s = w->io_h->io->synchronous;

    int lookup=0;
    int epeek;

    if (list = lm_utable_top(&ue_h->primary)) {
        if (lm_crawler_flag_isset(cr, LM_CRFLAG_EPEEK) && !ue_h->is_peeking)
            epeek = 1;
        else
            epeek = 0; /* epeek will be set to 0 if epeeking is disabled or 
                          if we are currently already epeeking an external host */

        for (x=0; x<list->sz; x++) {
            url = &list->row[x];

            if ((ft = lm_ftindex_match_by_url(&w->crawler->ftindex, url)) == 0) {
                lm_url_nullify(url);
                lm_url_swap(url, &list->row[list->sz-1]);
                list->sz--;
                x--;
                continue;
            }
            if (ft == LM_FTINDEX_POSSIBLE_MATCH) {
                if (s) {
                    ...
                    /* workaround mimetypes such as "text/html; charset=blah" */
                    char *c;
                    if (c = strchr(mime, ';'))
                        *c = '\0';
                    if (!(ft = lm_ftindex_match_by_mime(&w->crawler->ftindex, mime))) {
                        lm_url_nullify(url);
                        lm_url_swap(url, &list->row[list->sz-1]);
                        list->sz--;
                        x--;
            ...
            if (FT_FLAG_ISSET(ft, FT_FLAG_HAS_PARSER)) {
                ...
                if (LM_URL_ISSET(url, LM_URL_EXTERNAL)) {
                    /* The URL is external compared to the current hostname, 
                     * if epeek is set to 1, we must add this URL to the epeek list,
                     * because the next worker loop will enter an epeek state
                     * and temporarily crawl an external website. No internal 
                     * URLs will be added to the epeek list. */
                    if (!epeek) {
                        if (lm_crawler_flag_isset(cr, LM_CRFLAG_EXTERNAL))
                            ue_move_to_secondary(ue_h, url);
                    } else {
                        if (!peek_list) {
                            lm_utable_inc(&ue_h->primary);
                            peek_list = lm_utable_top(&ue_h->primary);

                            ue_h->depth_counter_bk = ue_h->depth_counter; /* back up values so we can return properly after the extenral peek */
                            ue_h->depth_limit_bk = ue_h->depth_limit;

                            ue_h->depth_counter = 0; /* zero depth counter, we will use the value
                                                        specified for the current crawler's peek limit as depth
                                                        limit instead, and base the depth from this point. */
                        ...
                        lm_url_swap(url, tmp);
                        /* swap the empty URL (now in the current list) with the 
                         * last non-null URL, to avoid holes in the list */
                        lm_url_swap(url, &list->row[list->sz-1]);
                        list->sz--; /* adjust the size counter due to last operation */
                        x--;
                        continue;
                    }
                    lm_url_nullify(url);
        ...
        if (lookup) {
                    ...
                    if (ft = lm_ftindex_match_by_mime(&w->crawler->ftindex, mime)) {
                        if (FT_FLAG_ISSET(ft, FT_FLAG_HAS_PARSER)) {
                            ...
                            if (LM_URL_ISSET(url, LM_URL_EXTERNAL)) {
                                ...
                                if (!epeek) {
                                    if (lm_crawler_flag_isset(cr, LM_CRFLAG_EXTERNAL)) {
                                        /* TODO: add to ue_h->parent->secondary */
                                    }
                                } else {
                                    if (!peek_list) {
                                        lm_utable_inc(&ue_h->primary);
            ...
            for (x=0; x<list->sz; x++) {
                if (!list->row[x].sz) {
                    lm_url_swap(&list->row[x], &list->row[list->sz-1]);
                    list->sz--;
                    x--;
                }
            }
